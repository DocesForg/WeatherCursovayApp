...
- `FavoriteCityController` — CRUD для избранных городов пользователя;
- `SupportController` — сообщения пользователя в поддержку и административные ответы;
- `RadioSignalController` — запуск теста радиосигнала и получение истории;
- `UserStatsController` — агрегированная статистика аккаунта;
- `AdminController` — административный dashboard и HTML-панель поддержки.
...
Бизнес-логика сервера распределена между сервисами и контроллерами:
...
- `RadioSignalController` кроме REST-обработки содержит расчёт дистанции по формуле haversine, потерь свободного пространства, задержки сигнала, примерной скорости и классификации качества канала. Для поправки на условия распространения он запрашивает текущую погоду в Open-Meteo через `RestClient`.
- `SupportController` формирует диалоги поддержки, отмечает сообщения прочитанными для администратора и разделяет пользовательские и администраторские сообщения.
- `UserStatsController` агрегирует количество избранных городов, тестов радиосигнала и обращений поддержки.
...
DTO реализованы Java records внутри контроллеров и `AccountDtos`. Они отделяют внешний JSON-контракт от JPA-сущностей и не раскрывают технические поля вроде `passwordHash`.
...
3. `RadioSignalController` рассчитывает расстояние, получает погодные поправки, вычисляет path loss, latency, speed и quality.
