...
Диаграммы последовательности описывают взаимодействие между объектами системы во времени для конкретных сценариев использования. Диаграммы показывают порядок обмена сообщениями, создаваемые объекты и условия выполнения операций.

**Диаграмма последовательности для сценария входа пользователя**

Данный сценарий описывает процесс аутентификации пользователя в системе. Пользователь вводит учётные данные на экране входа, которые передаются в ViewModel. ViewModel делегирует операцию UseCase классу, который взаимодействует с репозиторием аккаунтов. Репозиторий отправляет запрос на сервер через API клиент. Сервер проверяет учётные данные, генерирует JWT-токен и возвращает ответ. Токен сохраняется в хранилище сессий, и состояние UI обновляется для отображения авторизованного состояния.

<img src="/docs/04-detailed-design/images/seq1.png" alt="Диаграмма посследовательности 1">

**Диаграмма последовательности для сценария синхронизации избранных городов**

Сценарий описывает процесс загрузки и синхронизации списка избранных городов для авторизованного пользователя. При загрузке экрана избранного ViewModel инициирует получение списка через UseCase. UseCase запрашивает данные у репозитория, который проверяет наличие активной сессии. При наличии токена выполняется запрос к серверу для получения актуального списка. Сервер возвращает список городов, который сохраняется в локальном кэше и отображается пользователю.

<img src="/docs/04-detailed-design/images/seq2.png" alt="Диаграмма посследовательности 2">

**Диаграмма последовательности для сценария получения прогноза погоды**

Сценарий демонстрирует процесс загрузки и обработки прогноза погоды от внешнего API Open-Meteo. ViewModel запрашивает сводку погоды через UseCase, который координирует получение данных из репозитория прогнозов. Репозиторий проверяет актуальность закэшированных данных и при необходимости выполняет запрос к внешнему API. Полученные данные преобразуются в доменные модели и возвращаются для отображения.

<img src="/docs/04-detailed-design/images/seq3.png" alt="Диаграмма посследовательности 3">
