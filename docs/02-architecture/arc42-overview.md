...
|---|---------|--------|-------------|
| ADR-001 | Использовать PCMEF как архитектурную декомпозицию | Принято | Требуется разделить UI, управление, бизнес-логику, сущности и инфраструктуру. |
| ADR-002 | Использовать нативный Android + Jetpack Compose | Принято | Современный UI, Material 3, интеграция с Android SDK и Room. |
| ADR-003 | Использовать Spring Boot backend | Принято | Быстрая реализация REST, security, validation, JPA и OpenAPI. |
| ADR-004 | Использовать PostgreSQL и Spring Data JPA | Принято | Надежное хранение пользовательских данных и простой repository layer. |
| ADR-005 | Использовать JWT Bearer auth | Принято | Stateless backend и простая интеграция с мобильным клиентом. |
| ADR-006 | Использовать Open-Meteo как внешний погодный источник | Принято | Бесплатный API для прогноза, геокодинга и текущих погодных данных. |
