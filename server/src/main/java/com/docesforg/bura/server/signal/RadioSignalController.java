...
public class RadioSignalController {
    ...
    public RadioSignalController(RadioSignalTestRepository repository) {
    ...
    public RadioSignalResponse calculate(@PathVariable long accountId, @RequestBody RadioSignalRequest request) {
        double distanceKm = haversineKm(request.latitudeA(), request.longitudeA(), request.latitudeB(), request.longitudeB());
        double frequency = request.frequencyMhz() <= 0 ? 900.0 : request.frequencyMhz();
        WeatherSnapshot weatherA = getWeather(request.latitudeA(), request.longitudeA());
        WeatherSnapshot weatherB = getWeather(request.latitudeB(), request.longitudeB());
        ...
        entity.setCityA(request.cityA());
        entity.setCityB(request.cityB());
